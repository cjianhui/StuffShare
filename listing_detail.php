...
<?php
  ...
  $item_result = pg_query($connection,$query_item);
	...
	$query_bids = "SELECT MAX(bid_amount) AS max_bid, COUNT(*) AS count FROM bid where item_id=" . $_GET['id'];
...
<span><i class="lni-alarm-clock"></i> <?=$row['borrow_duration']?> day</span>
<span><i class="lni-map-marker"></i> <?=$row['address']?></span>
<span><i class="lni-eye"></i> <?=$bids['count']?> bids</span>
</div>
...
<div class="advertisement mb-2">
  <form method="POST" action="listing_detail.php?id=<?=$row['item_id']?>">
      <div class="input-group">
        ...
        <input name="bid_amt" type="number" class="form-control" min=<?=ceil($curr_min_bid)+1?> placeholder="<?=ceil($curr_min_bid)+1?> onwards">
